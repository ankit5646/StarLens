...
const Moviedetails = () => {
  ...
  useEffect(() => {
    dispatch(asyncloadmovie(id)) // Assuming id is a prop passed down from the parent component
    return () => {
      dispatch(removemovie()) // Remove the movie from the Redux store when the component unmounts
    }
  },[id])
  return info ? (
    <div
      ...
      className='w-full h-[120vh] px-[10%] relative'>
      ...
      <div className='flex gap-5 text-white'>
        ...
        <div>
          ...
          <p className='px-3 text-lg font-semibold'>{info.detail.release_date ? new Date(info.detail.release_date).getFullYear() : 'N/A'}
            <span className='px-5 text-base text-zinc-300'> {info.detail.runtime} min </span>
          </p> 
